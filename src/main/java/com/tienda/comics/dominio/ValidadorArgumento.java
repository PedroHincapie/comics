...
import com.tienda.comics.dominio.excepcion.*;
...
public final class ValidadorArgumento {
    ...
    public static void validarPositivoIncluyendoCero(Long valor, String mensaje) {
        if (valor < 0) {
            throw new ExcepcionValorInvalido(mensaje);
        }
    }

    public static void validarNumeroConLimite(Long valor, Long limite, String mensaje) {
        if (valor > limite) {
            throw new ExcepcionValorInvalido(mensaje);
        }
    }
    ...
    public static <T> void validarOpcional(T campo, Consumer<T> funcion) {
        if (campo != null) {
            funcion.accept(campo);
        }
    }

    public static void validarOpcionalNoRequerido(Object campo, String mensaje) {
        if (campo != null) {
            throw new ExcepcionValorNoNecesario(mensaje);
        }
    }

    public static <T> void validarCampoOpcional(T campo, Consumer<T> funcion) {
        if (campo != null) {
            funcion.accept(campo);
        }
    }
